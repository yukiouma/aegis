...
2. **Description rows start empty in create mode, regardless of `availableLanguages`.** The user adds description rows manually via the "Add description" button. This keeps the create drawer behavior identical whether or not the version already has domains — the user told us the drawer must start with zero rows.
...
- Update `useEffect` reset to branch on mode:
  ...
  - `mode === "create"` → `name=""`, `category="Special Purpose"`, `descriptions=[]`.
- `handleSubmit`:
...
- Render `<DomainEditDrawer>` with `mode`, `versionId={selectedVersionId}`, `canMutate`, plus the matching `onCreate` / `onUpdate` handlers. On success of either mutation, `setDomainDrawer(null)`.
...
- Manual smoke:
  ...
  4. Verify drawer opens with empty `Name`, default category, no description rows.
