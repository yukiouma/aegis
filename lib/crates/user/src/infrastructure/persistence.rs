...
//!
//! The infrastructure layer keeps the storage-specific code under
...
//! via the layer boundary (`infrastructure::UserRepo` and the crate
//! root), but the `postgres` child must be `pub` so the re-export at
//! the infrastructure layer is well-formed.
